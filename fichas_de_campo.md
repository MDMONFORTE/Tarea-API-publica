...
```
En la respuesta, vemos que el codigo devuelto es un 200, porque la petición se hace sin problemas, debido a que la key pasada es correcta. Sin embargo, el problema viene cuando la peticion en vez de introducir un año valido, introducimos una commilla. En la respuesta podemos ver que al API no rechaza la comilla por tipo, sino que lanza un mensaje de que los planes gratis, no tienen acceso a años fuera del intervalo del 2022 al 2024. Lo que refleja que probablemente el backend sanitiza las entradas de forma laxa. 
